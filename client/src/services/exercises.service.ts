...
export class ExercisesService {
  ...
  constructor(private userId: string, private serieId: number) {}

  public async fetchExercises(): Promise<Exercise[]> {
    try {
      const { data } = await Database.get(`exercise/serie/${this.serieId}`);
      this.setPointsPerExo(data[0].serie.level.pointsPerExo);
      return data;
    } catch (error) {
  ...
  public async markExerciseAsFailed(
    currentExercise: Exercise,
    completedExercises: Exercise[]
  ): Promise<void> {
    const exerciseId = currentExercise.id;
    ...
    if (
      !this.findExerciseInCompletedExercises(completedExercises, exerciseId)
    ) {
    ...
    if (networkObserver.isOffline()) {
      OfflineStorageService.store(
        `user-completed-exercise/${this.userId}/${exerciseId}`,
        {},
        methods.DELETE
      );
      return;
  ...
  private findExerciseInCompletedExercises(
    completedExercises: Exercise[],
    exerciseId: number
  ): Exercise {
  ...
  private async saveExerciseCompleted(exerciseId: number): Promise<void> {
    try {
      ...
      console.error(
        "Erreur lors de l'enregistrement de la complétion de l'exercice:",
        error
      );
    }
  }
  ...
  private async incrementFailedExercises(): Promise<void> {
    try {
      ...
      console.error(
        "Erreur lors de l'enregistrement de l'échec de l'exercice:",
        error
      );
    }
  }
