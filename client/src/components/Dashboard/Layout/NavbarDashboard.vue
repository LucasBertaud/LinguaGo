        ...
        <div class="flex flex-shrink-0 items-center ml-auto p-2">
            <button @click="openProfileModal"
                ...
                <div class="hidden md:flex md:flex-col md:items-end md:leading-tight">
                    <span class="font-semibold text-white group-hover:text-slate-600 group-focus:text-slate-600">{{
                        userInfo?.pseudo }}</span>
                    <span class="text-sm text-gray-300 group-hover:text-gray-500 group-focus:text-gray-500">{{
                        userInfo?.email }}</span>
                </div>
                <span
                    class="h-12 w-12 ml-2 sm:ml-3 mr-2 bg-gray-100 rounded-full overflow-hidden flex items-center justify-center">
                    <div v-if="avatarSvg" v-html="sanitizedSvg" class="h-12 w-12 flex items-center justify-center">
                    </div>
                </span>
