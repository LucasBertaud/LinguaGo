  ...
  <div class="flex flex-col row-span-3 bg-white shadow rounded-lg">
    ...
    <div class="p-4 flex-grow">
      <div
        ...
        v-if="topUsersOfWeek && topUsersOfWeek.length > 0"
      >
        <ul class="space-y-6">
          <li
            class="flex items-center"
            v-for="user in topUsersOfWeek"
            :key="user.pseudo"
          >
            <div
              class="h-10 w-10 mr-3 rounded-full overflow-hidden flex items-center justify-center"
...
const fetchTopUsersOfWeek = async () => {
  try {
    const { data } = await Database.get(
      "user-completed-exercise/top-users-of-week"
    );
    topUsersOfWeek.value = data;
  } catch (error) {
    console.error(error);
  }
};
