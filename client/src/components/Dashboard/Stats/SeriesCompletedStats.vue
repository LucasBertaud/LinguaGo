<template>
  <div class="flex flex-col row-span-3 bg-white shadow rounded-lg">
    ...
    <div class="p-4 flex-grow">
      <div
        v-if="seriesCompletedStats && !isNoSeriesCompleted"
...
const fetchSeriesCompletedStats = async () => {
  try {
    const { data } = await Database.get("user-completed-exercises-serie/user");
    const filterResponse: number[] = [];
    for (const level of levels) {
      filterResponse.push(
        data.filter((serie: any) => serie.exercisesSerie.level.title === level)
          .length
      );
    }
    seriesCompletedStats.value = filterResponse;
    isNoSeriesCompleted.value = data.length === 0;
  } catch (error) {
    console.error(error);
  }
};
