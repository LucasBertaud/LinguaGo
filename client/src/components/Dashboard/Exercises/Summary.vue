<template>
  <div class="container mx-auto p-4 bg-white rounded-lg shadow-lg max-w-4xl">
    <p class="text-lg mb-4">
      Vous avez fait tous les exercices pour cette série.
    </p>
    ...
    <div class="grid grid-cols-1 gap-4">
      <div
        ...
        :key="exercise.id"
        class="p-4 bg-gray-100 rounded-lg shadow transform transition-transform hover:scale-105"
      >
        <div class="flex items-center mb-2">
          <div
            class="flex-shrink-0 w-10 h-10 bg-primary text-white rounded-full flex items-center justify-center"
          >
            <svg
              class="w-6 h-6"
              fill="none"
    ...
    <p class="mt-4 text-lg font-semibold">
      ...
      <span class="text-green-500">{{ completedExercises.length }}</span> /
      <span class="text-gray-500">{{ totalExercises }}</span>
    </p>
    <p v-if="allCorrect" class="mt-4 text-lg font-semibold text-green-500">
      Félicitations ! Vous avez répondu correctement à toutes les questions !
    </p>
    <p v-else class="mt-4 text-lg font-semibold text-red-500">
      Vous n'avez pas répondu correctement à toutes les questions. Vous pouvez
      recommencer.
    </p>
    <button
      @click="goToExercises"
      class="mt-6 p-3 bg-primary text-white rounded-lg hover:bg-primary-dark transition duration-300 cursor-pointer"
    >
...
const allCorrect = () => {
  ...
  console.log(
    props.completedExercises.length,
    props.totalExercises,
    props.completedExercises
  );
  return props.completedExercises.length === props.totalExercises;
};
...
onMounted(() => {
  ...
  if (isAllCorrect) {
    ...
    const UserCompletedExercisesSerie: UserCompletedExercisesSerie[] =
      firstExercise.serie?.completedUsers || [];
