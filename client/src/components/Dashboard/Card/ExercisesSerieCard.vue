            ...
            :options="{
              dynamicClasses: `
                ...
                ${
                  ...
                  '[&>svg]:!fill-secondary !opacity-100'
                }`,
            }"
          />
          <h2 class="text-gray-900 text-lg title-font font-medium">
            {{ exercisesSerie.title }}
          </h2>
          <p class="text-gray-500 text-sm italic">
            Réussir {{ exercisesSerie.exercises.length > 1 ? "les" : "" }}
            {{ exercisesSerie.exercises.length }}
            {{ exercisesSerie.exercises.length > 1 ? "exercices" : "exercice" }}
            pour compléter la série.
          </p>
          <div
            class="w-full bg-gray-200 rounded-full h-3.5 dark:bg-gray-200 my-4"
          >
            <div
              class="bg-primary h-3.5 rounded-full"
              :class="percentageCompleted == 100 && '!bg-secondary'"
...
const props = defineProps<{
  ...
  routerParams?: { levelTitle: string; routerName?: string };
}>();
...
const userFavoriteSerie: UserFavoriteSerie | undefined =
  props.exercisesSerie.favoriteUsers.find(
    (favorite) =>
      favorite.userId === store.getters.getUser.id &&
      favorite.serieId === props.exercisesSerie.id
  );
...
const navigateToExercises = () => {
  router.push({
    name: props.routerParams?.routerName || "Exercises",
    params: {
