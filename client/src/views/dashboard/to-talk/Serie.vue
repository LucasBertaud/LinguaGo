<template>
  <div class="mb-8 w-full lg:mb-0 lg:w-[55%] lg:mr-16">
    <TalkExerciseCard
      @completed="serieCompleted = true"
      v-if="serie && !serieCompleted"
      :serie="serie"
      :points-per-exo="serie.level.pointsPerExo"
      :key="serie.id"
    />
    <div
      v-else
    ...
    >
      <div class="flex flex-col w-full justify-center items-center">
        <Icon
          ...
          :options="{
            dynamicClasses: '[&>svg]:w-26 [&>svg]:h-26 [&>svg]:fill-secondary',
          }"
        />
...
const fetchSerie = async () => {
  try {
    const { data } = await Database.get(`exercises-serie/${props.serieId}`);
    serie.value = data;
  } catch (error) {
...
const isSerieCompleted = (): boolean =>
  serie.value.completedUsers.some(
    (completedUser) => completedUser.userId === store.getters.getUser.id
  );
