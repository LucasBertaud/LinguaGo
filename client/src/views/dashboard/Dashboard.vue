<template>
  ...
  <section class="grid md:grid-cols-2 xl:grid-cols-4 gap-6">
    ...
    <SimpleStatCard
      ...
      icon="point-outline"
      :title="`${
        (userStats?.points || 0) > 1 ? 'Points gagnés' : 'Point gagné'
      }`"
    ...
    <SimpleStatCard
      ...
      icon="book-open"
      :title="`${
        (userStats?.successfullExercises || 0) > 1
          ? 'Exercises réussis'
          : 'Exercise réussi'
      }`"
      :stats="`${userStats?.successfullExercises || 0} / ${
        siteStats?.totalExercises || 0
      }`"
      icon-classes="[&>svg]:w-6 [&>svg]:h-auto"
    />
    <SimpleStatCard
      ...
      icon="completed"
      :title="`${
        (userStats?.completedSeries || 0) > 1
          ? 'Séries complétées'
          : 'Série complétée'
      }`"
      :stats="`${userStats?.completedSeries || 0} / ${
        siteStats?.totalSeries || 0
      }`"
      icon-classes="[&>svg]:w-6 [&>svg]:h-auto [&>svg>*]:fill-current"
    />
  </section>
  <section
    class="grid md:grid-cols-2 xl:grid-cols-4 xl:grid-rows-3 xl:grid-flow-col gap-6 mb-6"
  >
    ...
    <SimpleStatCard
      ...
      icon="clock"
      :title="`${
        timeStats?.getBestTimeTitle() || 'Temps'
      } passé sur les exercices`"
...
const fetchUserStats = async () => {
  try {
    ...
    const { data } = await Database.get("user-stats/user");
    timeStats.value = new TimeStatsUtils(data.timeSpentOnExercises);
...
const fetchSiteStats = async () => {
  try {
    isLoading.value = true;
    const { data } = await Database.get("site-stats");
