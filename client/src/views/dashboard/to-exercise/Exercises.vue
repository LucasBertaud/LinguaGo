<template>
  <div class="container mx-auto p-4" v-if="exercisesFetched">
    <div
      class="flex flex-row items-center mb-6 justify-between mx-4"
      :class="{ 'justify-center': allExercisesCompleted }"
    >
    ...
    <div v-else-if="currentExercise">
      <div class="mb-6 p-6 rounded-lg shadow-xl bg-white">
        <div class="flex flex-row">
          <h3 class="text-1xl font-semibold mb-4">
            {{ currentExerciseIndex + 1 }}. {{ currentExercise.question }}
          </h3>
        </div>
        ...
        <button
          @click="checkAnswer"
          class="mt-4 p-3 bg-primary text-white rounded-lg hover:bg-primary-dark transition duration-300 cursor-pointer"
        >
          Vérifier
        </button>
      </div>
...
const fetchExercises = async () => {
  ...
  exercises.value = await exercisesService.value
    .fetchExercises()
    .then((response: Exercise[]) => {
      completedExercises.value = response.filter(
  ...
  if (
    ...
    completedExercises.value.length === exercises.value.length
  ) {
...
const checkAnswer = async () => {
  ...
  const answer = sanitize(userAnswer.value);
  const correctAnswer = sanitize(exercise.answer);

  if (answer === correctAnswer) {
    isCorrect.value = true;
    await markExerciseAsCompleted(exercise);
  } else {
    if (exercise.type === "TRANSLATION") {
      const check = await translationCheck([exercise.question, answer]);
      if (check == "accept") {
        isCorrect.value = true;
        await markExerciseAsCompleted(exercise);
      } else {
        isCorrect.value = false;
        await markExerciseAsFailed(exercise);
      }
    } else {
      isCorrect.value = false;
      await markExerciseAsFailed(exercise);
...
const markExerciseAsCompleted = async (exercise: Exercise) => {
  isLoading.value = true;
  exercisesService.value
    .markExerciseAsCompleted(exercise, completedExercises.value)
    .then(() => {
      if (!completedExercises.value.includes(exercise)) {
        completedExercises.value.push(exercise);
      }
    })
    .finally(() => {
      isLoading.value = false;
...
const markExerciseAsFailed = async (exercise: Exercise) => {
  isLoading.value = true;
  exercisesService.value
    .markExerciseAsFailed(exercise, completedExercises.value)
    .then(() => {
      if (completedExercises.value.includes(exercise)) {
        completedExercises.value = completedExercises.value.filter(
          (exercise: Exercise) => exercise.id !== exercise.id
        );
      }
    })
    .finally(() => {
