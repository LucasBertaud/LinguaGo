...
export class NotificationController {
  constructor(
    private readonly genericService: GenericService<Notification>,
  ...
  async create(
    ...
    try {
      const existingNotification = await this.genericService.findOne(
        'notification',
        {
          where: {
            userId: String(req.user.id),
  ...
  async update(
    ...
    if (
      updateNotificationDto.notificationTime ||
      updateNotificationDto.frequency
    ) {
      updateNotificationDto.nextNotifyAt =
        await this.notificationService.updateNextNotifyAt(
          String(req.user.id),
          updateNotificationDto.frequency,
          updateNotificationDto.notificationTime,
        );
