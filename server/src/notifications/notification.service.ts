...
export class NotificationService {
  ...
  async triggerNotifications() {
    ...
    notifications.forEach(async (notification) => {
        ...
        const nextNotifyAt: Date = await this.updateNextNotifyAt(notification.userId, notification.frequency, notification.notificationTime);
  ...
  private enumValueToInt(value: string): number {
    switch (value) {
      case 'TODAY' :
        return 0;
      default:
  ...
  public async updateNextNotifyAt(userId: string, frequencyDto: string | undefined, notificationTimeDto: string | undefined): Promise<Date> {
    ...
    const frequencyNumberOfDay: number = this.enumValueToInt(frequency);

    const notificationTime: string = notificationTimeDto || currentNotification.notificationTime;
